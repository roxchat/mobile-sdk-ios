...
class AbstractRequestLoop {
    ...
    func perform(request: URLRequest) throws -> Data {
        ...
        requestWithUserAgent.setValue("iOS: Roxchat-Client 3.0.0; (\(UIDevice.current.model); \(UIDevice.current.systemVersion)); Bundle ID and version: \(Bundle.main.bundleIdentifier ?? "none") \(Bundle.main.infoDictionary?["CFBundleVersion"] ?? "none")", forHTTPHeaderField: "User-Agent")
