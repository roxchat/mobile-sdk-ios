...
class DeltaRequestLoop: AbstractRequestLoop {
    ...
    override func start() {
        ...
        queue = DispatchQueue(label: "ru.roxchat.DeltaDispatchQueue")
        guard let queue = queue else {
            RoxchatInternalLogger.shared.log(entry: "DispatchQueue initialisation failure in DeltaRequestLoop.\(#function)")
            return
        }
        queue.async {
            self.run()
    ...
    func requestInitialization() {
        ...
        request.setValue("3.0.0", forHTTPHeaderField: Parameter.roxchatSDKVersion.rawValue)
    ...
    func requestDelta() {
        guard let url = URL(string: getDeltaServerURLString() + "?" + getDeltaParameterString()) else {
            RoxchatInternalLogger.shared.log(entry: "Initialize URL failure in  DeltaRequestLoop.\(#function)")
            return
        }
        var request = URLRequest(url: url)
        request.httpMethod = AbstractRequestLoop.HTTPMethods.get.rawValue
    ...
    private func handleInitialization(error: String) {
        ...
        case RoxchatInternalError.reinitializationRequired.rawValue:
            ...
            break
        case RoxchatInternalError.providedAuthenticationTokenNotFound.rawValue:
    ...
    private func handleProvidedAuthenticationTokenNotFoundError() {
        ...
        if DeltaRequestLoop.providedAuthenticationTokenErrorCount < 5 {
            ...
            guard let token = providedAuthenticationToken else {
                RoxchatInternalLogger.shared.log(entry: "Provided Authentication Token is nil in  DeltaRequestLoop.\(#function)")
                return
            }
