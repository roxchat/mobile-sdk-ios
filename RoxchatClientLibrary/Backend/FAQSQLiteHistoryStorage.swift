    ...
    private func createTableWith(name: String) {
        FAQSQLiteHistoryStorage.queryQueue.sync { [weak self] in
            ...
            guard let libraryPath = optionalLibraryDirectory else {
                RoxchatInternalLogger.shared.log(entry: "Error getting access to Library directory.",
            ...
            do {
                ...
                RoxchatInternalLogger.shared.log(entry: "Creating Connection(\(dbPath) failure in FAQSQLiteHistoryStorage.\(#function)")
                return
            }
        }
    }
